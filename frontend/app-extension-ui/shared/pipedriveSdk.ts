...
export function usePipedriveSdk(surface: Surface) {
  ...
  useEffect(() => {
    ...
    async function initialize(): Promise<void> {
      try {
        ...
        const instance = new AppExtensionsSDK({
        ...
        const initializedSdk = await instance.initialize({
          size: initialSize(surface),
        });
        if (cancelled) return;
  ...
  const runSdkAction = useCallback(
    async <T>(
      ...
      action: (client: AppExtensionsSDK) => Promise<T>,
    ): Promise<T | undefined> => {
      if (!sdk) {
  ...
  const showConfirmation = useCallback(async () => {
    const response = await runSdkAction('Confirmation answered', (client) =>
      client.execute(Command.SHOW_CONFIRMATION, {
        ...
        description:
          'This dialog is rendered by Pipedrive through the App Extensions SDK.',
        okText: 'Confirm',
        cancelText: 'Cancel',
  ...
  const getSignedToken = useCallback(async () => {
    const response = await runSdkAction('Signed token received', (client) =>
      client.execute(Command.GET_SIGNED_TOKEN),
    );

    if (response) {
      setSignedTokenPreview(
        response.token.slice(0, 16) + '...' + response.token.length + ' chars',
      );
    }
  }, [runSdkAction]);
